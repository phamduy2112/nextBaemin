...
import { getUserById } from "@/app/api/fetchApi";

export default function HeaderNav() {
    const router = useRouter();
    const onSearch: SearchProps['onSearch'] = (value, _e, info) => {
        router.push(`/sreach?key=${value}`)
    ...
    useEffect(()=>{
        const fetchCartItems = () => {
          ...
          if (typeof window !== 'undefined') { // Check if window object exists (client-side)
    ...
    return (
        <div className="w-full h-fix bg-white flex flex-row fixed  py-3 gap-4 justify-items-center	justify-center z-50	">
            ...
            {hasToken ? <Button href="/login" 
            onClick={logout}
            className="font-normal  leading-5 btn-home	" style={{ fontSize: '18px',height:'100%' ,color:'rgb(128, 128, 137)' }}  type="text" icon={<SolutionOutlined />}>Logout</Button>
:            <Button href="/login" 

className="font-normal  leading-5 btn-home	" style={{ fontSize: '18px',height:'100%' ,color:'rgb(128, 128, 137)' }}  type="text" icon={<SolutionOutlined />}>Tài Khoản</Button>
}
            ...
            <span className="text-xs bg-red-600 relative rounded w-full text-white  bottom-3 right-4 text-center" style={{width:'15px' ,borderRadius:'50px'}}  >{totalItems ||0}</span>
            </div>
