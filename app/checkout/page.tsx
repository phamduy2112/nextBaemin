...
import { getProductAll, getStoreAll, getUserById, postOrder } from "../api/fetchApi";
...
const [product,setProduct]=useState([])
    ...
    const getUser=async()=>{
        ...
        if(resp.message=="Thành công !"){
            setUser(resp.content)
        }

    }   
    const getProduct=async()=>{
        const resp=await getProductAll();
    ...
    const handleOnclick = async () => {
        ...
        const list_sanPham = cartItems.map(item => ({ "id": item.id, "sl": item.quantity }));
        let listSP = JSON.stringify(list_sanPham);
        ...
        const resp=await postOrder(newPost)
        if(resp=="Thành công"){
            alert("Đạt hàng thành công thành công")
            ...
            <div className="px-16 flex flex-col gap-3 ">'
                <div className="w-full h-28 flex flex-col bg-white rounded-md pl-10 pt-5">
                    ...
                    <div className="pl-3 flex flex-row gap-5 items-center mb-3 mt-3">
                        <span className="font-bold" >{user?.lastname} {user?.firstname} (+84) {user?.phone} </span>
                ...
                <div className="w-full bg-white rounded-md  flex flex-col pt-5 ">
                    ...
                    <DetailsCheckout groupedItems={groupedItems} data={data}/>
                ...
                <div className="w-full  flex flex-col bg-white rounded-md  pt-5 gap-3">
                    ...
                    <div className="w-full border-t  flex flex-row justify-between items-center  pt-4  gap-4 mb-4">
                        ...
                        <div  className="w-[30%] pl-48 ">
                            <button onClick={handleOnclick} className="p-1 bg-beamin text-white w-36 rounded-md h-10 hover:brightness-105">
                                Đặt hàng
                            </button>
                        </div>
                    </div>
